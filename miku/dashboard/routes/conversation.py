...
class ConversationRoute(Route):
            ...
            if "conversations" in data:
                ...
                for conv in conversations:
                    ...
                    try:
                        ...
                        self._purge_profile_artifacts(cid)
                        deleted_count += 1
                    except Exception as e:
    ...
    def _purge_profile_artifacts(self, cid: str) -> None:
        """删除会话时清理画像旁路：ConversationStore + MBTI 缓存。
        失败不影响主流程。
        """
        if not cid:
            return
        try:
            ...
            get_store().delete_sessions_by_cid(cid)
        except Exception as e:
            logger.warning(f"清理 ConversationStore 失败 (cid={cid}): {e!s}")
        try:
            import os
            import re
            from miku.core.utils.astrbot_path import get_astrbot_data_path
            profile_dir = os.path.join(get_astrbot_data_path(), "profile")
            if not os.path.isdir(profile_dir):
                return
            for fn in os.listdir(profile_dir):
                if not fn.startswith("mbti_") or not fn.endswith(".json"):
                    continue
                if cid in fn or re.sub(r"[^A-Za-z0-9_.-]", "_", cid) in fn:
                    try:
                        os.remove(os.path.join(profile_dir, fn))
                    except OSError:
                        pass
        except Exception as e:
            logger.warning(f"清理 MBTI 缓存失败 (cid={cid}): {e!s}")
