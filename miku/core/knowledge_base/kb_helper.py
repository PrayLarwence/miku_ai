...
from miku.core.db.vec_db.base import BaseVecDB
from miku.core.exceptions import KnowledgeBaseUploadError
...
if TYPE_CHECKING:
    from miku.core.db.vec_db.faiss_impl.vec_db import FaissVecDB
    from .memory_model import MemoryModelManager
...
class KBHelper:
    ...
    def __init__(
        ...
        kb: KnowledgeBase,
        memory_model_manager: "MemoryModelManager",
        kb_root_dir: str,
        chunker: BaseChunker,
        ...
        self.kb_db = kb_db
        self.kb = kb
        self.memory_model_mgr = memory_model_manager
        self.prov_mgr = provider_manager  # for LLM-based cleaning features
    ...
    async def get_ep(self) -> EmbeddingProvider:
        ...
        ep: EmbeddingProvider = await self.memory_model_mgr.get_provider_by_id(
    ...
    async def get_rp(self) -> RerankProvider | None:
        if not self.kb.rerank_provider_id:
            return None
        rp: RerankProvider | None = await self.memory_model_mgr.get_provider_by_id(
    ...
    async def upload_from_url(
        ...
        """
        # 获取 Tavily API 密钥
        if self.prov_mgr is None:
            raise ValueError(
                "Error: ProviderManager is required for URL upload."
            )
