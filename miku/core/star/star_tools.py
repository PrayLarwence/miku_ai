...
from miku.api.platform import AstrBotMessage, MessageMember, MessageType
from miku.core.message.components import BaseMessageComponent
from miku.core.message.message_event_result import MessageChain
from miku.core.platform.astr_message_event import MessageSesion
# Platform adapters removed - webchat only
...
class StarTools:
    ...
    async def send_message_by_id(
        ...
        if platform == "webchat":
            adapter = next(
                (p for p in platforms if True),
                None,
            )
            if adapter is None:
                raise ValueError("未找到 WebChat 适配器")
            await AstrMessageEvent.send_message(
                bot=adapter.bot,
